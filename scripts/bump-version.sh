...
# AWS OpenTelemetry Android SDK Version Bump Script
# This script bumps the version in gradle.properties and README.md
...
get_current_version() {
    if [[ ! -f "$GRADLE_PROPERTIES" ]]; then
        print_color $RED "Error: $GRADLE_PROPERTIES not found!"
        exit 1
    fi
    
    # Extract version from gradle.properties
    local version=$(grep "^version=" "$GRADLE_PROPERTIES" | cut -d'=' -f2)
    
    if [[ -z "$version" ]]; then
        print_color $RED "Error: Could not find version in $GRADLE_PROPERTIES"
        exit 1
    fi
...
update_version_in_file() {
    ...
    if [[ ! -f "$file_path" ]]; then
        print_color $YELLOW "Warning: $file_path not found, skipping"
        return 0
    fi
...
update_version_in_gradle_properties() {
    ...
    local new_pattern="version=$new_version"
