...
Build the image:
```bash
docker build -t contract-test-endpoint .
```

Run the container:
```bash
docker run -p 8080:8080 contract-test-endpoint
```

## Docker Compose

This service is included in the Docker Compose file at `tests/contract-tests/compose.yaml` for use in contract testing.
